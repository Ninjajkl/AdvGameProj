...
{
	[Tooltip("The max deep and height of the desert dunes, low values")][Range(0, TerrainConstants.MAX_HEIGHT - 1)]
	public int maxHeightDifference = TerrainConstants.MAX_HEIGHT / 5;

	[Tooltip("Number vertex (y), where the snow end and the rock start")][Range(0, TerrainConstants.MAX_HEIGHT - 1)]
	public int snowDeep = TerrainConstants.MAX_HEIGHT / 5;
	...
	[Tooltip("Ice columns max height")][Range(0, TerrainConstants.MAX_HEIGHT - 1)]
	...
	public override byte[] GenerateChunkData(Vector2Int vecPos, float[] biomeMerge)
	{
		...
		for (int z = 0; z < TerrainConstants.CHUNK_VERTEX_SIZE; z++)
		{
			for (int x = 0; x < TerrainConstants.CHUNK_VERTEX_SIZE; x++)
			{
				...
				//Ice calculations
				int iceExtraHeigh = 0;
				if (iceNoise[x + z * TerrainConstants.CHUNK_VERTEX_SIZE] > iceApearValue)
					iceExtraHeigh = Mathf.CeilToInt((1- iceNoise[x + z * TerrainConstants.CHUNK_VERTEX_SIZE] ) / iceApearValue * iceMaxHeight);
